...
def main():
    ...
    ser = serial.Serial(SERIAL_PORT, 115200)  # Replace 'COM3' with your serial port and baud rate
    ...
    while True:
        ...
        if rawFrame[-2:]==[13, 10]:
            ...
            if len(rawFrame) == 16:
                # x_gyro 2bytes, y_gyro 2bytes, z_gyro 2bytes, pressure 4bytes, temperature 4bytes
                (x_gyro, y_gyro, z_gyro) = struct.unpack('>hhh', bytes(rawFrame[:-10]))

                reassembledTemperature = struct.unpack('>f', bytes([rawFrame[6], rawFrame[7], rawFrame[8], rawFrame[9]]))[0]

                output = 'gyr_x={0:<6} gyr_y={1:<6} gyr_z={2:<6}'.format(x_gyro, y_gyro, z_gyro)
                print(output)
                print(reassembledTemperature)
